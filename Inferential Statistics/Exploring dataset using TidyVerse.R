...
movies <- read.csv("C:/Users/shiva/Documents/MS Data Science/631-Foundations of Data Analysis/Class2/Movies.csv") #Must specify an exact file path
...
title_col = select(movies, Title) # data.frame

title_col
# Try these four with the tibble.
# What's different?
tidy.movies[,1] #List of top 10 values and ... for other rows.
tidy.movies[1]
tidy.movies$Opening.Gross
select(tidy.movies, Title)

# Which movies were released in 2014? 

# View only 2014 movie titles

movies[12:20,2]  # Note that 1:30 creates a list of the numbers from 1 to 30 inclusive. This works any time a list is called for
...
select(filter(movies,Franchise == 1),Title,Genre)
# 4) Display only Title and Genre of Franchise movies that were Oscar Nominated
select(filter(movies,Franchise == 1,Oscar.Nominated == 1),Title,Genre)
...
#Q: Will mean > median?
mean(movies$Budget)
median(movies$Budget)  # mean > median because it's right skewed
